...
if ( $mode eq 'dssp' ) {
	...
	foreach my $pdb_id (@pdb_id_list)
	{
		my $hash_ref = processDSSP( $pdb_id );
...
#wegen nur einem element aufruf string in array einspeichern
...
# letzte sequenz in arrays
		...
		$hash{id_seq}        = \@id_seq;
		return \%hash;
# TODO methode nicht nur einmal hash zurückgeben
	}
}
