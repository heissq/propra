	...
	public static void main(String[] args) throws IOException, ParseException {
		...
		if (cmd.hasOption("method")) {
			if (cmd.getOptionValue("method").equals("gor1")) {
				type = 1;
			} else if (cmd.getOptionValue("method").equals("gor3")) {
				type = 3;
			}
		} else {
		...
		// String filename =
		// "/home/proj/biocluster/praktikum/bioprakt/progprakt6/Solution4/test.txt";
		...
		if (type == 3) {
			...
			for (int i = 0; i < prim.length; i++) {
				...
				p[i] = new Sequence(prim[i].getid(), prim[i].getps(),
						prediction);
			}
		} else {
	...
	public Sequence2[] predictTrain(Sequence[] topredict, Sequence[] tomodel, boolean gor3) throws FileNotFoundException {
		...
		if (gor3) {
			...
			for (int i = 0; i < topredict.length; i++) {
				p[i] = new Sequence2(topredict[i].getid(), topredict[i].getps(),
						g.predictString(topredict[i].getps()), topredict[i].getss());
			}
		} else {
			Gor1Model g = new Gor1Model();
			g.train2(tomodel);
