...
public class Gor4Model {
	...
	// C,E,H -- Aminosäure -- Aminosäure an Stelle -- Stelle -- Aminosäure in
	// Tabelle - Position
	...
	// Anzahl der Vorkommen der Sekundärstrukturen im Modell für AA pspred in
	// der Mitte, AA ps
	// an Position pos
	...
	public double[] prob(int[] ps, int pos) {
		...
		for (int g = 0; g < windowsize; g++) {
			if (true) {
				...
				for (int i = g + 1; i < windowsize; i++) {
					p[0] += model[0][ps[pos]][ps[pos + g - whalf]][g][ps[pos
							+ i - whalf]][i];
					p[1] += model[1][ps[pos]][ps[pos + g - whalf]][g][ps[pos
							+ i - whalf]][i];
					p[2] += model[2][ps[pos]][ps[pos + g - whalf]][g][ps[pos
							+ i - whalf]][i];
				}
				r[0] += Math.log(p[0] / (double) (p[1] + p[2]));
				r[1] += Math.log(p[1] / (double) (p[0] + p[2]));
				r[2] += Math.log(p[2] / (double) (p[1] + p[0]));
				q[0] += Math.log(summatrix(model, ps[pos], ps[pos + g - whalf],
						g)[0]
						/ (double) (summatrix(model, ps[pos], ps[pos + g
								- whalf], g)[1] + summatrix(model, ps[pos],
									ps[pos + g - whalf], g)[2]));
				q[1] += Math.log(summatrix(model, ps[pos], ps[pos + g - whalf],
						g)[1]
						/ (double) (summatrix(model, ps[pos], ps[pos + g
								- whalf], g)[0] + summatrix(model, ps[pos],
									ps[pos + g - whalf], g)[2]));
				q[2] += Math.log(summatrix(model, ps[pos], ps[pos + g - whalf],
						g)[2]
						/ (double) (summatrix(model, ps[pos], ps[pos + g
								- whalf], g)[1] + summatrix(model, ps[pos],
									ps[pos + g - whalf], g)[0]));
		...
		p[0] = (2.0 / windowsize) * r[0] * (windowsize - 2.0) / (windowsize)
				* q[0];
		p[1] = (2.0 / windowsize) * r[1] * (windowsize - 2.0) / (windowsize)
				* q[1];
		p[2] = (2.0 / windowsize) * r[2] * (windowsize - 2.0) / (windowsize)
				* q[2];
		if (p[0] > p[1] && p[0] > p[2])
			p[3] = 0;
