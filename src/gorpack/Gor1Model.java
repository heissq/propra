...
public class Gor1Model implements GorModel {
	...
	public void makematrix(int[][][] mod) {
		...
		for (int i = 0; i < nstates; i++) {
			for (int j = 0; j < naa - 1; j++) {
				for (int k = 0; k < windowsize; k++) {
					this.matrix[i][j][k] = Math
							.log((double) mod[i][j][k]
									/ (mod[0][j][k] + mod[1][j][k]
											+ mod[2][j][k] - mod[i][j][k]))
	...
	/*
	 * //returns percent value public int[] prob2(int[] ps, int pos){ int[] perc
	 * = new int[3]; double[] p = new double[3]; for(int i = 0; i < windowsize;
	 * i++){ p[0] += Math.exp(matrix[0][ps[pos+i-whalf]][i]); p[1] +=
	 * Math.exp(matrix[1][ps[pos+i-whalf]][i]); p[2] +=
	 * Math.exp(matrix[2][ps[pos+i-whalf]][i]); } perc[0] = (int)
	 * ((double)p[0]/(p[1]+ p[2]+ p[0])*10); perc[1] = (int)
	 * ((double)p[1]/(p[0]+ p[2]+ p[1])*10); perc[2] = (int)
	 * ((double)p[2]/(p[1]+ p[0]+ p[2])*10); return perc; }
	 */
	...
	/*
	 * public int[][] predictProbs(int[] ps){ int[][] r = new int[3][ps.length];
	 * for(int x = 0; x < whalf; x++){ r[0][x] = 3; r[1][x] = 3; r[2][x] = 3;
	 * r[0][ps.length - x -1] = 3; r[1][ps.length - x -1] = 3; r[2][ps.length -
	 * x -1] = 3; } for(int i = whalf; i < ps.length-whalf; i++){ double[] p =
	 * prob(ps, i); int[]probs = prob2(ps, i); r[0][i] = probs[0]; r[1][i] =
	 * probs[1]; r[2][i] = probs[2]; //System.out.println("foobar"); }
	 * 
	 * return r; }
	 */
