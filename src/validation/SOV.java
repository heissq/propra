...
public class SOV {
	...
	private double sumOv_NonOv() {
		...
		for (DoubleSegment dbseg : dbl_segments)
			sumov += dbseg.observed.endpos - dbseg.observed.startpos;

		for (Segment seg : non_overlap_segments)
			sumnonov += seg.endpos - seg.startpos;
	...
	private double sumOv() {
		...
		for (DoubleSegment dbseg : dbl_segments) {
			...
			if (dbseg.observed.startpos <= dbseg.predicted.startpos
				...
				minov = dbseg.predicted.endpos - dbseg.predicted.startpos;
				maxov = dbseg.observed.endpos - dbseg.observed.startpos;
			} else if (dbseg.observed.startpos > dbseg.predicted.startpos
					&& dbseg.observed.endpos < dbseg.predicted.endpos) {
				minov = dbseg.observed.endpos - dbseg.observed.startpos;
				maxov = dbseg.predicted.endpos - dbseg.predicted.startpos;
			} else if (dbseg.observed.startpos > dbseg.predicted.startpos) {
				minov = dbseg.predicted.endpos - dbseg.observed.startpos;
				maxov = dbseg.observed.endpos - dbseg.predicted.startpos;
			} else {
				minov = dbseg.observed.endpos - dbseg.predicted.startpos;
				maxov = dbseg.predicted.endpos - dbseg.observed.endpos;
			}
			del = delta(minov, maxov,
					dbseg.observed.endpos - dbseg.observed.startpos,
					dbseg.predicted.endpos - dbseg.predicted.startpos);
			sum += ((minov + del) / maxov)
					* (dbseg.observed.endpos - dbseg.observed.startpos);
		}
		return sum;
